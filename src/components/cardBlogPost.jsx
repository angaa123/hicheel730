...
const baseURLWithTags = (tags) => `https://dev.to/api/articles?tag=${tags.join(",")}`;
...
function CardBlogPost() {
    ...
    React.useEffect(() => {
        async function fetchArticles() {
            try {
                const url = tags.length > 0 ? baseURLWithTags(tags) : baseURL;
    ...
    return (
        <div>
            <ul className="cardContainer flex  flex-wrap gap-y-5 gap-5 md:flex-row items-center md:justify-between md:w-full ">
                                ...
                                <div className="bottom-2 max-w-96 w-min min-w-60">
                                    <div className="flex">
                                        {article.tag_list.map((tag) => (
                                            <p key={tag} className="bg-indigo-700 w-max py-1 px-2 text-white text-sm font-medium rounded-xl mb-4">
                                                {tag}
                                            </p>
                                        ))}
                                    </div>
